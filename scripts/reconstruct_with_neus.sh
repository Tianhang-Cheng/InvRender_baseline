...
if ! [[ -f "$neus_data_dir/exp/womask_sphere/meshes/00300000.ply" ]]; then
        ...
        python exp_runner.py --mode train --conf "$neus_data_dir/womask.conf" --case "dummy" $options
        if [[ -f "$neus_data_dir/exp/womask_sphere/meshes/00300000.ply" ]]; then
                python exp_runner.py --mode validate_mesh --conf "$neus_data_dir/womask.conf" --case "dummy" --is_continue 
        fi
fi
popd
