...
def theta_intrinsics_calibration(args):
    ...
    ## solvepnp does not seem to work for some reason
    # for name, data in (('left',left), ('right',right)):
    #     points2d = []
    #     points3d = []
    ...
    #             points2d.append(corner)
    #             points3d.append(keypoints[f'{tag.family}.{tag.id:02d}.{corner_i}'])
    #     points2d = np.array(points2d, dtype=np.float32)[None,...]
    #     points3d = np.array(points3d, dtype=np.float32)

    #     undist_points2d = cv2.fisheye.undistortPoints(points2d, data.best_K, data.best_dist_coeff)

    #     retval, rvec, tvec = cv2.solvePnP(points3d, undist_points2d[0], np.eye(3), np.zeros((1,5)), flags=cv2.SOLVEPNP_EPNP)
    #     R,_ = cv2.Rodrigues(rvec)
    #     visualize_camera(R, tvec[:,0], f'cam_calib/extrinsics_{name}', time=0)
    ...
    #             img_points.append(np.array(points2d, dtype=np.float32)[None,...])
    #             obj_points.append(np.array(points3d, dtype=np.float32)[None,...])
    #             if i == best_i:
    #                 # remember the index for the output here
    #                 ext_i = i

    #     im = data.images[0]
    #     flags = cv2.fisheye.CALIB_FIX_SKEW | cv2.fisheye.CALIB_FIX_K1 | cv2.fisheye.CALIB_FIX_K2 | cv2.fisheye.CALIB_FIX_K3 | cv2.fisheye.CALIB_FIX_K4 | cv2.fisheye.CALIB_FIX_INTRINSIC
    #     rms, K, dist_coeff, rvecs, tvecs = cv2.fisheye.calibrate(obj_points, img_points, im.shape[::-1], K, dist_coeff, flags=flags | cv2.fisheye.CALIB_USE_INTRINSIC_GUESS)

    #     R,_ = cv2.Rodrigues(rvecs[ext_i])
    #     visualize_camera(R, tvecs[ext_i][:,0], f'cam_calib/extrinsics_{name}', time=0)
