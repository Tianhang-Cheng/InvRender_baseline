...
def relit_with_light(model, relit_dataloader, relight_index, images_dir, 
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(relit_dataloader):
        ...
        if data_index not in relight_index:
            continue
        ...
        out_img_name = '{}'.format(str(indices[0].item()).zfill(4))
        ...
        mask = plt.lin2img(mask, img_res)[0].permute(1, 2, 0)
        ...
        if light_type == 'origin':
            rgb_relit_ = model_outputs['sg_rgb'] + model_outputs['indir_rgb']
        else:
            rgb_relit_ = model_outputs['sg_rgb'] 

        rgb_relit = decode_img(rgb_relit_, batch_size, total_pixels, img_res, is_tonemap=True)
        ...
        rgb_relit_env_bg.save('{0}/pr_image_{1}.png'.format(images_dir, out_img_name))
        all_frames.append(np.array(rgb_relit))

        rgb_relit = decode_img(rgb_relit_, batch_size, total_pixels, img_res, is_tonemap=False)
        bg_mask = ~object_mask.expand(-1,-1,3).cpu().numpy()
        rgb_relit[bg_mask] = bg_rgb[bg_mask]
        with open('{0}/pr_image_{1}.npy'.format(images_dir, out_img_name), 'wb') as f:
            np.save(f, rgb_relit) 
...
def relight_obj(**kwargs):
    ...
    env_path = kwargs['data_split_dir'].replace('dataset_neus','dataset')

    # import pdb
    # pdb.set_trace()

    envmap6_path = os.path.join(env_path, 'test', 'gt_env_512_rotated_0000')
    print('Loading light from: ', envmap6_path)
    model.envmap_material_network.load_light(envmap6_path)
    images_dir = relitdir + '/test'
    utils.mkdir_ifnotexists(images_dir)
    print('Output directory is: ', images_dir)
    relit_with_light(model, relit_dataloader, [0,1,2], images_dir, 
                     total_pixels, img_res, albedo_ratio=None, light_type='origin')
    
    # import pdb
    # pdb.set_trace()
    
    envmap12_path = os.path.join(env_path, 'test', 'gt_env_512_rotated_0003')
    print('Loading light from: ', envmap12_path)
    model.envmap_material_network.load_light(envmap12_path)
    images_dir = relitdir + '/test'
    utils.mkdir_ifnotexists(images_dir)
    print('Output directory is: ', images_dir)
    relit_with_light(model, relit_dataloader, [3,4,5], images_dir, 
                    total_pixels, img_res, albedo_ratio=None, light_type='envmap2')

    envmap12_path = os.path.join(env_path, 'test', 'gt_env_512_rotated_0006')
    print('Loading light from: ', envmap12_path)
    model.envmap_material_network.load_light(envmap12_path)
    images_dir = relitdir + '/test'
    utils.mkdir_ifnotexists(images_dir)
    print('Output directory is: ', images_dir)
    relit_with_light(model, relit_dataloader, [6,7,8], images_dir, 
                    total_pixels, img_res, albedo_ratio=None, light_type='envmap3')
