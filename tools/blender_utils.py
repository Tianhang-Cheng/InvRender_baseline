...
class SavePosePriors(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        envname = im_path.parent.parent.name
        objname = im_path.parent.parent.parent.name
        source_path = Path(cams[0]['image_path']).parent.parent.parent.parent/'source_data'/objname/envname

        for cam in cams:
            cvcam = CVCamera.from_blender(cam, cam['image_height'], cam['image_width'])
            
            print(cam.matrix_world)
            print(np.array(cam['original_intrinsics']))
            print(source_path)
