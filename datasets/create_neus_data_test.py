...
def create_neus_data(input_dir: Path, output_dir: Path): 
    ...
    camera_paths = sorted(list(input_dir.glob('gt_camera_*.txt')))
    image_paths = sorted(list(input_dir.glob('gt_image_*.png')))
    assert len(camera_paths) == len(image_paths) and len(image_paths)

    bounds = np.loadtxt(input_dir/'inputs/object_bounding_box.txt')
...
if __name__ == '__main__':
    ...
    if not output_dir.exists() or args.overwrite:
        create_neus_data(input_dir, output_dir)
    else:
