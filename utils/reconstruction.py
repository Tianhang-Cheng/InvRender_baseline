...
def init_colmap_reconstruction(output_dir: Path, image_dir: Path, overwrite: bool=False):
    ...
    # sift_opts.num_octaves = 5
    # pycolmap.extract_features(db_path, image_dir, sift_options=sift_opts)

    exhaustive_matcher(output_dir)
    # sift_match_opts = pycolmap.SiftMatchingOptions()
    # sift_match_opts.max_error = 12
    # pycolmap.match_exhaustive(db_path, sift_options=sift_match_opts)
...
def run_mapper(output_dir: Path, image_dir: Path):
    ...
    status = subprocess.check_call(map(str,cmd), shell=False)
    print(status)
    # return pycolmap.Reconstruction(output_dir/'0')
...
def visualize_reconstruction(recon: pycolmap.Reconstruction, imgid_colmapkpid_kp=None, sg_prefix='recon'):
    ...
    for k, p in recon.points3D.items():
        ...
        if imgid_colmapkpid_kp is not None and el.point2D_idx in imgid_colmapkpid_kp[el.image_id]:
            ...
            # keypt = imgid_colmapkpid_kp[el.image_id][el.point2D_idx]
            # if keypt.id.startswith('obj'):
            #     print(keypt.id, tuple(p.xyz))
            #     for tel in p.track.elements:
            #         is_custom_keypoint = tel.point2D_idx in imgid_colmapkpid_kp[tel.image_id]
            #         print(tel, recon.images[tel.image_id].name, is_custom_keypoint)
        else:
            points.append(p.xyz)
...
def write_neus_config_file(output_dir: Path):
    ...
    with open(output_dir/'womask.conf','w') as f:
