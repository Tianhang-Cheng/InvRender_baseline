...
def plot_mat(model_outputs, rgb_gt, path, iters, img_res):
    ...
    x = (clip_img(tonemap_img(sg_rgb)).reshape([img_res[0],img_res[1],3]).detach().cpu().numpy() * 255).astype(np.uint8)
    img = Image.fromarray(x)
    img.save('{0}/{1}.png'.format(path,iters))

    x = sg_rgb.reshape([img_res[0],img_res[1],3]).detach().cpu().numpy()
    imageio.imsave('{0}/{1}.exr'.format(path,iters), x) 
